...
interface Props {
  ...
  /** Accepted for pi-app call-site compatibility; the slash palette is always enabled here. */
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const thinkingLevelDesc: Record<(typeof THINKING_LEVELS)[number], string> = {
    auto: t("chatInput.thinkingLevelDefault"),
    off: t("chatInput.thinkingLevelOff"),
    minimal: t("chatInput.thinkingLevelMinimal"),
    low: t("chatInput.thinkingLevelLow"),
    medium: t("chatInput.thinkingLevelMedium"),
    high: t("chatInput.thinkingLevelHigh"),
    xhigh: t("chatInput.thinkingLevelXhigh"),
    max: t("chatInput.thinkingLevelMax"),
  };
  ...
  const slashCommandCountLabel = filteredSlashCommands.length === 1
    ...
    : `${filteredSlashCommands.length} ${slashQuery ? t("chatInput.slashMatches") : t("chatInput.slashCommandsWord")}`;
  ...
  useEffect(() => {
    ...
    const el = slashItemRefs.current[slashActiveIndex];
  ...
  const compactVerb = compactResult?.reason && compactResult.reason !== "manual"
    ? t("chatInput.compactedWithReason", { reason: `${compactResult.reason[0].toUpperCase()}${compactResult.reason.slice(1)}` })
    : t("chatInput.compacted");
  const compactResultText = compactResult
    ? t("chatInput.compactResultSummary", {
        verb: compactVerb,
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{
                ...
                {t("chatInput.queuedLabel", { count: (queuedMessages?.steering.length ?? 0) + (queuedMessages?.followUp.length ?? 0) })}
              </span>
              {onRecallQueue && (
                <button
        ...
        <div style={{ position: "relative" }}>
          {slashMenuOpen && slashQuery !== null && (
            ...
            >
              ...
              >
                <span>{slashCommandsLoading ? t("chatInput.slashLoading") : t("chatInput.slashCommandsHeader", { label: slashCommandCountLabel })}</span>
          ...
          {atMenuOpen && atQuery !== null && (() => {
            ...
            const matchCountLabel = atMatches.length === 1
              ? t("chatInput.filesMatchOne")
              : t("chatInput.filesMatchMany", { count: atMatches.length });
            // With a truncated index, local results are provisional — the
            // debounced server search over the full listing replaces them.
            const truncatedHint = fileIndex?.truncated && !serverResultInUse
              ? (atQuery.query ? t("chatInput.filesSearchingAll") : t("chatInput.filesIndexTruncated"))
              : "";
            return (
              <div
                style={{
              ...
              >
                ...
                >
                  <span>
                    {indexLoading
                      ? t("chatInput.filesLoading")
                      : t("chatInput.filesHeader", { label: `${matchCountLabel}${truncatedHint}` })}
                  </span>
                  <span style={{ fontFamily: "var(--font-mono)" }}>Tab / Enter</span>
                </div>
                <div style={{ maxHeight: "calc(min(48vh, 400px) - 34px)", overflowY: "auto", padding: 4 }}>
                  {!indexLoading && atMatches.length === 0 ? (
                    <div style={{ padding: "6px 8px", fontSize: 12, color: "var(--text-dim)" }}>
                      {needsServerSearch && !serverResultInUse ? t("chatInput.filesSearching") : t("chatInput.filesNoMatch")}
                    </div>
                  ) : (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0, alignSelf: "flex-end" }}>
              {onSteer && (
                <button
                  ...
                  title={attachedImages.length ? t("chatInput.queueImagesBlocked") : t("chatInput.steerTitle")}
                  style={{
              ...
              {onFollowUp && (
                <button
                  ...
                  disabled={!canQueueStreamingMessage}
                  title={attachedImages.length ? t("chatInput.queueImagesBlocked") : t("chatInput.followUpTitle")}
                  style={{
                    display: "flex", alignItems: "center", gap: 5,
                    padding: "7px 12px",
            ...
            {!isStreaming && onThinkingLevelChange && (
              <div ref={thinkingDropdownRef} style={{ position: "relative" }}>
                <button
                  ...
                  disabled={isStreaming}
                  title={t("chatInput.changeThinkingLevel", { level: thinkingDisplayLabel })}
                  aria-label={t("chatInput.changeThinkingLevelAria")}
                  style={{
            ...
            {!isStreaming && onToolPresetChange && (
              <div ref={toolDropdownRef} style={{ position: "relative" }}>
                <button
                  ...
                  disabled={isStreaming}
                  title={t("chatInput.changeToolPreset", { preset: toolPresetLabel })}
                  aria-label={t("chatInput.changeToolPresetAria")}
                  style={{
                    display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
                    padding: isMobile ? "0 6px" : "8px 12px",
                    width: isMobile ? "auto" : undefined,
            ...
            {!isStreaming && onCompact && (
              <div style={{ position: "relative" }}>
                ...
                <button
                  ...
                  aria-label={isCompacting ? t("chatInput.stopCompacting") : t("chatInput.compactContext")}
                >
                  {isCompacting ? (
