...
// Pre-commit guard: blocks `package-lock.json` changes unless explicitly allowed.
// Mirrors /Users/mk/codespace/pi/scripts/check-lockfile-commit.mjs, adapted for
// pi-web (single-package, no workspaces).
//
// Usage:
//   git commit ...                # blocks if package-lock.json is staged
//   PI_APP_ALLOW_LOCKFILE_CHANGE=1 git commit ...   # explicit override

import { execFileSync } from "node:child_process";

const allowValue = process.env.PI_APP_ALLOW_LOCKFILE_CHANGE;
...
if (allowed) {
	console.error("package-lock.json is staged; PI_APP_ALLOW_LOCKFILE_CHANGE is set, allowing commit.");
	process.exit(0);
}
...
console.error("  - confirm npm age gates were active for resolution (min-release-age=2 in .npmrc)");
...
console.error("  PI_APP_ALLOW_LOCKFILE_CHANGE=1 git commit ...");
process.exit(1);
