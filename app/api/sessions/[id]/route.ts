...
export const GET = withTenant(async (
  ...
  try {
    ...
    const parentSessionId = header?.parentSession
      ? await resolveSessionIdByPath(header.parentSession, currentAgentDir())
      : undefined;
...
export const DELETE = withTenant(async (
  ...
  try {
    ...
    // Ownership check before any filesystem mutation. Prefer SessionManager so
    // malformed files still follow the same denial path as GET/PATCH.
    try {
      const sm = SessionManager.open(filePath);
      const ownerRejected = rejectLivoCwdOutsideWorkspace(req, sm.getHeader()?.cwd);
      if (ownerRejected) return ownerRejected;
    } catch {
      // Fall through to header read / existing delete error behavior.
    }

    // Read only the bounded header before deleting.
    const deletedHeader = readSessionHeader(filePath);
    const parentSessionPath = deletedHeader?.parentSession;
    const cwd = deletedHeader?.cwd;
