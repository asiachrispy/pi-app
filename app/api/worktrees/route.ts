...
import { allowFileRoot, getAllowedFileRoots, isFilePathAllowed, isRealPathAllowed } from "@/lib/file-access";
...
async function checkCwdAllowed(cwd: string): Promise<NextResponse | null> {
  ...
  // Realpath (symlink-resolved) containment for paths that still exist. A
  // removed-worktree cwd may no longer be on disk, in which case the lexical
  // check above already authorized it and GET falls back to the project root.
  if (existsSync(cwd) && !isRealPathAllowed(cwd, allowedRoots)) {
    return NextResponse.json({ error: "Access denied" }, { status: 403 });
  }
  return null;
}
