...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const loadSlashCommands = useCallback(async () => {
    if (!sessionIdRef.current && isNew && newSessionCwd) {
      setSlashCommandsLoading(true);
      try {
        const res = await fetch(`/api/slash-commands?cwd=${encodeURIComponent(newSessionCwd)}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json() as SlashCommandsResponse;
  ...
  }, [ensureNewSession, isNew, newSessionCwd]);
  ...
  useEffect(() => {
    ...
    setSlashCommands([]);
    void loadSlashCommands();
  }, [isNew, newSessionCwd, loadSlashCommands]);
